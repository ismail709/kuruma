...
import MapBookingSchema from "../../../../helpers/schema-validation/MapBookingSchema";
import Modal from "./book-car-modal/modal";
...
function Map() {
    ...
    // show modal when a car is clicked
    const [showModal, setshowModal] = useState(false);

    //handle book car click event
    const handleBookCar = async () => {
        await MapBookingSchema.validate({
            start: new Date(
                startdate.current.value + " " + starttime.current.value
            ),
            end: new Date(enddate.current.value + " " + endtime.current.value),
        })
            .then((val) => {
